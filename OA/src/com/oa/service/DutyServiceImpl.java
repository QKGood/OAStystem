...
public class DutyServiceImpl implements DutyService {
	...
	@Override
	public void addAddress(String address,String empId) {
		String addressid = dutyDAO.queryNullAddress();
		if(addressid!=null){
			String pattern = "(\\d{1}$)";
			String empCol = "empid";
			Pattern r = Pattern.compile(pattern);
			Matcher m = r.matcher(addressid);
			 if (m.find( )) {
				 empCol+=m.group(0);
			 }else{
				 System.out.println("NO MATCH");
			 }
			dutyDAO.addAddress(addressid,address,empCol,empId);
		}else {
			Duty duty = new Duty();
			duty.setWeekday("1");
