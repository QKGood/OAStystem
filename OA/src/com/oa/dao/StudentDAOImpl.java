...
public class StudentDAOImpl extends AbstractBaseDAO<Student> implements StudentDAO {
	...
	@Override
	public long countEmpID(String empId,String staStus) {
		Session session = sessionFactory.getCurrentSession();
	     Query query = session.createQuery("select  count(*) from Student t where t.empId = ? and t.stuStatus = ?");
	     query.setString(0, empId);
	     query.setString(1, staStus);
	     long count = (long) query.uniqueResult();
	     return count;
	}
	...
	public Student login(Student stu) {
		Session session = sessionFactory.getCurrentSession();
		 Query query = session.createQuery("from Student e where e.phone = :stuPhone and e.pwd = :pwd");
		 query.setParameter("stuPhone", stu.getPhone());
		 query.setParameter("pwd", stu.getPwd());
	...
	@Override
	public boolean phone(Student stu) {
		Session session = sessionFactory.getCurrentSession();
		 Query query = session.createQuery("from Student e where e.phone = :stuPhone");
		 query.setParameter("stuPhone", stu.getPhone());
		 stu = (Student) query.uniqueResult();
			if (stu != null) {
				return true;
	...
	@Override
	public void updatePwd(String stuId,String pwd) {
		Session session = getSessionFactory().getCurrentSession();
		String sql = "update t_stu s set s.pwd = :pwd where s.stuid = :stuId";
