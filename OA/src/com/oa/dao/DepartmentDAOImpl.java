...
public class DepartmentDAOImpl extends AbstractBaseDAO<Department> implements DepartmentDAO{
	...
	public String queryDepNameByDepId(String depid){
		Session session = getSessionFactory().getCurrentSession();
		String sql = "select name from t_dept where depid= ?";
		SQLQuery query= session.createSQLQuery(sql);
	...
	@Override
	public String queryDepIdByDepName(String depname) {
		Session session = getSessionFactory().getCurrentSession();
		String sql = "select depid from t_dept where name= ?";
		SQLQuery query= session.createSQLQuery(sql);
		query.setParameter(0,depname);
		return 	(String) query.uniqueResult();
	}

	@Override
	public String queryDepidByEmpNor(String empid) {
		Session session = getSessionFactory().getCurrentSession();
		String sql = "select e.depid from t_dept d right join t_emp e on e.depid = d.depid where e.empid = ?";
		SQLQuery query= session.createSQLQuery(sql);
		query.setParameter(0,empid);
		return 	(String) query.uniqueResult();
	}
