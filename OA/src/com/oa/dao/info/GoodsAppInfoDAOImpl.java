...
public class GoodsAppInfoDAOImpl extends AbstractBaseDAO<GoodsAppInfo> implements GoodsAppInfoDAO {
	...
	public Pager4EasyUI<GoodsAppInfo> queryPager(String beanName, Pager4EasyUI<GoodsAppInfo> pager) {
		...
		String sql = "select ga.goodsappid, ga.appday, ga.quantity, ga.des, ga.'status', ga.appstatus,ga.price, "
				+ " e.empid, e.name as empName, g.goodsId, g.'name' as goodsname "
        ...
        for(Object[] o: row){
        	...
        	gi.setPrice((double) o[6]);
        	gi.setEmpId(o[7].toString());
        	gi.setEmpName(o[8].toString());
        	gi.setGoodsName(o[9].toString());
        	list.add(gi);
        }
        pager.setRows(list);
		return pager;
