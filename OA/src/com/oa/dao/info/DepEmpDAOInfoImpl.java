...
public class DepEmpDAOInfoImpl extends  AbstractBaseDAO<DepEmp>  implements DepEmpDAOInfo{
	public Pager4EasyUI<DepEmp> query( Pager4EasyUI<DepEmp> pager){
        ...
        Query query = session.createQuery("select d.depId,d.name,d.des,e.empId,e.name ,d.status from Department d left join d.employee e where d.status = 1");
	...
	public int countPager() {
		Session session = getSessionFactory().getCurrentSession();
        Query query = session.createQuery("select count(d.depId) from Department d left join d.employee e where d.status = 1");
