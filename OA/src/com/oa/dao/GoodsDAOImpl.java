...
public class GoodsDAOImpl extends AbstractBaseDAO<Goods> implements GoodsDAO {
	...
	public Pager4EasyUI<GoodsInfo> queryInfoPager(Pager4EasyUI<GoodsInfo> pager) {
		...
		Query query = session.createSQLQuery("select g.goodsid, g.name as goodsName,g.quantity,"
				+ "g.des,g.unitprice,g.buyday,g.status,gt.goodstypeid,gt.name as goodsTypeName "
				+ "from t_goods g ,t_goodstype gt where g.goodstypeid = gt.goodstypeid");
	...
	public void updateGoods(Goods goods) {
		...
		Query query = session.createSQLQuery("update t_goods set name=?, quantity=?, des=?, unitprice=?, "
				+ "goodstypeid=?, status=? , buyday=? where goodsId=?");
		...
		query.setParameter(6, goods.getBuyDay());
		query.setParameter(7, goods.getGoodsId());
		query.executeUpdate();
	}
...
//		return pager;
	...
	@Override
	public void updateQuantityByName(String beanName, String goodsName, int quantity) {
		String hql = "update " + beanName + " as gNames set quantity =:quantity where gNames.name =:gName";
		Session session = getSessionFactory().getCurrentSession();
		Query query = session.createQuery(hql);
		query.setInteger("quantity", quantity);
		query.setString("gName", goodsName);
		query.executeUpdate();
	}
