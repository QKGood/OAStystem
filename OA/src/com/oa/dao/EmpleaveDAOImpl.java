...
public class EmpleaveDAOImpl extends AbstractBaseDAO<Empleave> implements EmpleaveDAO{
	...
	public Pager4EasyUI<EmpleaveInfo> baseQuery(Pager4EasyUI<EmpleaveInfo> pager, String depId, String empId,
		...
		StringBuffer whe = new StringBuffer(" where ea.leaveday<= :endLeave ");
		...
		if(startBegin!=null){	whe.append(" and ea.starttime >= :startBegin ");	}
		if(startEnd != null) {	whe.append(" and ea.starttime <= :startEnd ");	}
		if(endBegin!=null){		whe.append(" and ea.endtime >= :endBegin ");	}
		if(endEnd != null) {	whe.append(" and ea.endtime <= :endEnd");	}
		if(beginLeave!=null){	whe.append(" and ea.leaveday >= :beginLeave ");	}
		if(endLeave==null){ 	Date curDate = new Date(); curDate.setHours(23);endLeave= curDate;}
		
		if(sort!=null&&!sort.equals("")){
			orderby+= " order by "+ sort + " " + order;
			
		}else {
			orderby+=" order by pass asc, leaveday desc ";
		}
		
		if(depId!=null && !depId.equals(""))	{		whe.append(" and d.depid=:depid " );		}
		if(empId != null &&!empId.equals(""))	{		whe.append(" and ea.empid=:empid ");		}
		...
		if(secondlevel!=null && secondlevel.equals("1")){//第二级已审核,一定是审核过的,如果想找没有审核的就在else里找
			whe .append(" and ea.secondlevel= 1 ");
		}else {//第二级未审核
			if(firstlevel!=null && firstlevel.equals("1")){//判断第一级是否已经审过了
				//已经审过了
				whe.append(" and ea.firstlevel = 1 ");
				if(secondlevel!=null && secondlevel.equals("0")){whe.append(" and ea.secondlevel = 0");}
			}else if(firstlevel!=null && firstlevel.equals("0")){//第一级也未审过
				whe.append(" and ea.firstlevel = 0 ");
				if(secondlevel!=null && secondlevel.equals("0")){whe.append(" and ea.secondlevel = 0");}
			}
				//第一级未审核
		}
		...
		if(hasPass!=null&&!hasPass.equals("0")&&!hasPass.equals("")){//是否需要过滤pass,为0则不需要过滤,也就是是否审核到真正结果都无所谓,用于单人用户
			if(pass!=null&&!pass.equals("")){//需要过滤pass,也就是说要么是查询审核完的,要么就是没有审核过的
				if(pass.equals("1")){		//审核过的
					whe.append(" and ea.pass = 1 ");
				}else if(pass.equals("2")){
					whe.append(" and ea.pass = 2 ");
				}
			}else {
				whe.append(" and ea.pass !=0 ");
			}
		...
		if(status!=null && status.equals("")){
			whe.append(" and ea.status=" + Integer.valueOf(status) + " ");
		}
		
		String sql = sel + fro + addFro +whe.toString() + orderby ;
	...
	@Override
	public int baseCount( String depId, String empId,String empName, 
		...
		int count  = 0;
		if(obj!=null) 
		{			BigInteger bigInteger = (BigInteger)obj; count = bigInteger.intValue();		}
	...
	public void baseUpdSecondLeave(String leaveid,int pass) {
		String sql = "update t_empleave set secondlevel= 1 ,firstlevel = 1 , pass = ? , status = 1 where leaveid = ?";
		Session session = getSessionFactory().getCurrentSession();
		SQLQuery query = session.createSQLQuery(sql);
		query.setParameter(0, pass);
	...
	public int countNewStatus(String empId) {
		String sql = "select count(ea.leaveId) from Empleave ea left join ea.employee  e where e.empId = :empId  and ea.status = 1";
		Session session = getSessionFactory().getCurrentSession();
	...
	public void updLeaveStatus(String empId) {
		String sql = "update t_empleave  set status = 0 where empId = :empId  ";
		Session session = getSessionFactory().getCurrentSession();
		SQLQuery query = session.createSQLQuery(sql);
		query.setParameter("empId", empId);
		query.executeUpdate();
