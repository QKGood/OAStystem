...
public class EmpCheckingDAOImpl extends AbstractBaseDAO<EmpChecking> implements EmpCheckingDAO{
	...
	public Pager4EasyUI<EmpCheckingStatistics> queryCheck4Base(Pager4EasyUI<EmpCheckingStatistics> pager,String depid,List<EmpCheckingInfo> norms,String empid,String empName,Date begin,Date end,String format,String sort, String order) {
		...
		for(int i = 0; i<num ;i++) {//打卡次数限制于info,所以需要循环num
			...
			}else {		late.append("(case when ec.time"+(i+1)+" !='' and (ec.time"+(i+1)+" < ? or ec.time"+(i+1)+" > ?) then 1 else 0 end))/case when count(DISTINCT(el.leaveid)) >0 then count(DISTINCT(el.leaveid)) else 1 END ,");
			}
		...
		String fro= "  from t_emp e left join t_empchecking ec   on ec.empid=e.empid left join t_dept d on e.depid = d.depid ";
		String leave = "left join (select empid ,leaveid from  t_empleave where starttime>=:starttime and endtime<=:endtime and pass = 2 ) as el on e.empid = el.empid ";
		...
		if(empName!=null&&!empName.equals("")){empName = WebUtil.vague(empName);where.append(" and e.name like :empName");}
	...
	public List<EmpCheckingVOInfo> queryByIds(String[] ids) {
        ...
        if(result!=null) {
        	...
        	for(int i = 0,len=result.size(); i<len; i++) {
        		...
        		voChecking.setDepName(obj[0].toString());
        		voChecking.setEmpId(obj[1].toString());
        		voChecking.setEmpName(obj[2].toString());
	...
	public int countCheck4Base(String depid,String empid,String empName,Date begin,Date end) {
		//TODO
		String sql = "select count(DISTINCT e.empid) ";
		...
		Session session = sessionFactory.getCurrentSession();
	    Query query = session.createSQLQuery(sql +  fro + where);
	    int j = 1;
	...
	public int countInfo(String depid,String empid, String empName, Date beginDate, Date endDate) {
		...
		String wheDate= " ec.checkingDay <= :endDate";
		if(beginDate!=null) {			wheDate += " and ec.checkingDay  >= :beginDate ";		}
		String sql  = "select count(ec.checkingId) from t_empchecking ec left join t_emp e on ec.empid = e.empid left join t_dept d on e.depid = d.depid where " + wheDate ;
		...
		Session session = sessionFactory.getCurrentSession();
	        ...
	        if(countObj!=null){
	        	BigInteger bigInteger = (BigInteger) countObj ;
