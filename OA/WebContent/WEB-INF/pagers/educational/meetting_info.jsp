...
<script>
	...
	function showAddMeettingWin(){
		authority('com.oa.action.MeettingAction.meettingSave', "", "add",'');
	}
	...
	function showEditMeettingWin(){
		...
		if(rows.length  > 1 ){
			$.messager.alert("提示","请选择一个要修改的会议","info");
			...
			if(row){
				...
				authority('com.oa.action.MeettingAction.meettingUpdate', json1, "edit",'');
			}
	...
	function submitDepWin(){
		...
		if(rows.length  > 1 ){
			$.messager.alert("提示","请选择一个会议","info");
		}else if(rows.length == 1){
			var row = rows[0];
			if(row){
	...
	function disable(index){   // 禁用
		authority('com.oa.action.MeettingAction.meettingDisable', '', "disable",index);
	}
		
	function activate(index){  // 启用
		authority('com.oa.action.MeettingAction.meettingEnabled', '', "activate",index);
	}
...
<table id="list" class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'meettingDay',width:150" formatter="formatterDate">会议时间</th>
				<th data-options="field:'createdDay',width:150" formatter="formatterDate">添加时间</th>
	...
	<div id="tb" style="padding:5px;height:auto">
		...
		<div>
			主持人/会议详情/会议时间: <input id="emppName" class="easyui-textbox" style="width:80px">
			<a href="javascript:;" onclick = "queryByName()" class="easyui-linkbutton" iconCls="icon-search">搜索</a>
		</div>
	</div>
	
	<div id="addWin" class="easyui-window" title="添加研讨会" data-options="closed:true" style="width:350px;height:300px;">
		<form id="addForm">
			<table>
				<tr>
	...
	<div id="editWin" class="easyui-window" title="修改会议" data-options="closed:true" style="width:500px;height:300px;">
		<form id="editForm">
			<table>
				<tr>
