...
<script>
	...
	function showAddProgressWin(){
		authority('com.oa.action.ProgressAction.progressSave', "", "add",'');
	}
	function showEditProgressWin(){
		var rows = $("#list").datagrid("getSelections");
		if(rows.length  > 1 ){
			$.messager.alert("提示","请选择一个要修改的课程进度","info");
			...
			if(row){
				...
				authority('com.oa.action.ProgressAction.progressUpdate', json1, "edit",'');
			}
	...
	function disable(index){   // 禁用
		authority('com.oa.action.ProgressAction.progressDisable', '', "disable",index);
	}
		
	function activate(index){  // 启用
		authority('com.oa.action.ProgressAction.progressEnabled', '', "activate",index);
	}
...
<table id="list" class="easyui-datagrid"
		...
		<thead>
			...
			<th data-options="field:'progressId',checkbox:true">课程进度编号</th>
				...
				<th data-options="field:'progressDay',width:140,align:'center'" formatter="formatterDate">添加进度的时间</th>
