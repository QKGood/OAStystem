...
<script>
	...
	function showAddCourseWin(){
		authority('com.oa.action.CourseAction.courseSave', "", "add",'');
	 }
	function showEditCourseWin(){
		...
		if(rows.length  > 1 ){
			...
			if(row){
				...
				authority('com.oa.action.CourseAction.courseUpdate', json1, "edit",'');
			}
		}else if(rows.length == 0){
			$.messager.alert("提示","请选择一个要修改的课程","info");
		}
	...
	function disable(index){   // 禁用
		authority('com.oa.action.CourseAction.courseDisable', '', "disable",index);
	}
		
	function activate(index){  // 启用
		authority('com.oa.action.CourseAction.courseEnabled', '', "activate",index);
	}
