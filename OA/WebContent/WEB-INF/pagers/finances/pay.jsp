...
<script>
	...
	var jsonStr = '{"pi.payId":"_payid","pi.empName":"_empname","pi.payTypeName":"_paytypename","pi.payDay":"_payday","pi.des":"_des","pi.pay":"_pay","pi.toName":"_toname","emp.name":"_name","pi.toPhone":"_tophone","pi.payTypeStatus":"_paytypestatus"}';
	
	var jsonStr2 = '{"pay.payId":"_payid","pay.empId":"_empid","pay.payTypeId":"_paytypeid","pay.payDay":"_payday","pay.des":"_des","pay.pay":"_pay","pay.toName":"_toname","emp.name":"_name","pay.toPhone":"_tophone"}';
	...
	function editPayInfoWin() {//修改
		...
		if(rows.length > 1) {
			...
			if(row) {// 有选中的行
				...
				var jsonStr4 = jsonStr2.replace("_payid",row.payId).replace("_empid",row.empId).replace("_paytypeid",row.payTypeId).replace("_payday",row.payDay).replace("_des",row.des).replace("_pay",row.pay).replace("_toname",row.toName).replace("_tophone",row.toPhone).replace("_name",row.name);
...
<body>
	...
	<div id="addwin" class="easyui-window" title="添加" data-options="closed:true" style="width:330px;height:380px;">
		<form id="addForm">
			<table>
				...
				<tr>
					<td >员工姓名: </td> 
					<td><input id="empName" class="easyui-validatebox textbox" name="pi.empName"/>
					  <a name="pi.empName" id="Employee" onclick = "selectEmployee()">选择</a> 
					</td>
				</tr>	
				<tr>
					<td>支出类型</td>
				...
				<tr>
					<td>支出时间: </td>
					<td><input class="easyui-datetimebox" name="pay.payDay"></input></td>
				</tr>
				<tr>
					<td>描述: </td>
				...
				<tr>
					<td>支付宝号码: </td>
					<td><input class="easyui-validatebox easyui-textbox" name="pay.toPhone"  data-options="required:true,validType:'length[11,11]',missingMessage:'请输入支付宝账号',invalidMessage:'请输入正确的账号'" /></td>
				</tr>
				<tr>
