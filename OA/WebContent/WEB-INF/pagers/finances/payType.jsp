...
<script>
	...
	function showAddPayTypeWin() {
		authority('com.oa.action.PayTypeAction.addPayType', "", "add",'');
	}
	...
	function showEditPayTypeWin() {
		...
		if (rows.length > 1) {
			$.messager.alert("提示", "请先选择一个需要修改的类别", "info");
			...
			if (row) { // 有选中的行
				...
				authority('com.oa.action.PayTypeAction.updatePayType', jsonStr1, "edit",'');
			}
	...
	function disable(index){   // 禁用
		authority('com.oa.action.PayTypeAction.payTypeDisable', '', "disable",index);
	}
		
	function activate(index){  // 启用
		authority('com.oa.action.PayTypeAction.payTypeActivate', '', "activate",index);
	}
...
<body style="height:100%;">
	...
	<div id="editWin" class="easyui-window" title="修改支出类别" data-options="closed:true" style="width:500px;height:300px;">
		<form id="editForm">
