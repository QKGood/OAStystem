...
<style type="text/css">
	...
	/* 动态更新使用css开始 */ 
	 ...
	 .total{ width:10px;height:10px;position:absolute;top:20px;background-color:red;border-radius:50%;}
	 #userWork:after{
	 	margin-left:10px;
	 	content:"|";
	 	color:#ccc;
...
<body class="easyui-layout">
		...
		<div style = "position:absolute;left:500px;top:20px;"><span style = "font-size:20px;">当前时间：<span id = "div1" style = "font-size:20px;"></span></span></div>
			...
			<!-- 程燕,测试中 开始 -->
			<!-- 程 燕 动态更新申诉状态  开始 -->
					<div id="userWork" style="margin-right:30px" >个人业务<span class= "total"></span></div>
					<div id="userWorkMenu" class="easyui-menu" style="width:120px;">
					    <div onclick="workIsLook('考勤申诉管理','<%=path %>/empAppeal/showEmpAppeal?self=self','empAppeal_total');" class= "empAppeal_status">我的申诉<span class= "empAppeal_total empWorkStatus hide"></span></div>  
					    <div onclick="workIsLook('员工请假管理','<%=path %>/empleave/showEmpleave?self=self','empLeave_total');" class= "empLeave_status">我的请假<span class= "empLeave_total empWorkStatus hide"></span ></div>  
					</div>
					<script>
					$(function(){
						var sessionEmp = str2JsonStr("${session.employee}");
							var workTag = $("#userWork");
							var workMenu = $("#userWorkMenu");
							if(sessionEmp!=null){
								sessionEmp.role = str2JsonStr("${session.employee.role}");
								sessionEmp.department = str2JsonStr("${session.employee.department}");
								var roleName = sessionEmp.role.name;
								if(roleName != "总经理"){
									$("#userWork").bind('mouseover',userWorkFun);
									$("#userWork").bind('mousemove',userWorkFun);
									
									getWorkStatus("<%=path%>/empleave/newStatus","empLeave_status","empLeave_total");
								 	getWorkStatus("<%=path%>/empAppeal/newStatus","empAppeal_status","empAppeal_total");
								 	setTimeout("totalFun()",500);
								 	...
								 	window.setInterval("totalFun()",6000);
								}else{
									workTag.remove();
									workMenu.remove();
									console.log("是总经理")
						...
						function userWorkFun(e){
							var userWork = $("#userWork")
							$(this).css("cursor","pointer")
							e.preventDefault();
							$('#userWorkMenu').menu('show', {
								left: userWork.offset().left,
								top: userWork.offset().top+userWork.outerHeight()-20
							});
						};
						function workIsLook(title,url,iconClass){
							$("."+iconClass).addClass("hide").removeClass("show");
							$("."+iconClass).text("");
							totalFun();
							addTab(title,url);
						}
					 	...
					 	function getWorkStatus(url,className,totalClassName){
					 		$.get(url,function(data) {
					 			console.log(data.countNewStatus);
					 			console.log("aaa")
					 			if(data.countNewStatus!=null&&data.countNewStatus!=''){
						 				$("." + totalClassName).addClass("show").removeClass("hide");
						 				$("."+totalClassName).text(data.countNewStatus);
					 			}
					 		})
					 	}
					 	
					 	function totalFun(){
				 			if(($(".empLeave_total").text()!=null&&$(".empLeave_total").text()!="")||($(".empAppeal_total").text()!=null&&$(".empAppeal_total").text()!="")){
					 			$(".total").addClass("show").removeClass("hide");
				 			}else{
				 				$(".total").addClass("hide").removeClass("show");
				 			}
				 		};
					</script>
			<!-- 程燕,测试中  结束-->
			<!--程 燕 动态更新申诉状态  结束-->
				...
				<a href = "javascript:;" style = "color:blue;" onclick="exit();">安全退出</a>
			</div>
		</div>
		...
		<div data-options="region:'west',split:true" title="菜单" style="width:235px; ">
			<div class="easyui-accordion menu" style="width:235px;">
				<div title="市场管理" class="menu1"  data-options="iconCls:'icon-hamburg-statistics'" style="padding:10px;">
					<ul>
				...
				<div title="办公管理" class="menu1" data-options="iconCls:'icon-hamburg-business'" style="padding:10px;">
					<ul>
						...
						<li><a href="javascript:;" onclick="addTab('值班管理','<%=path %>/duty/showDuty');">值班管理</a></li>
						<li><a href="javascript:;" onclick="addTab('巡查管理','<%=path %>/check/showCheck');">巡查管理</a></li>
				...
				<div title="教务管理" class="menu1" data-options="iconCls:'icon-hamburg-order'" style="padding:10px;">
					<ul>
				...
				<div title="财务管理" class="menu1" data-options="iconCls:'icon-hamburg-bank'" style="padding:10px;">
					<ul>
		...
		<div data-options="region:'center'">
			<div id="tabs" class="easyui-tabs" style="width:100%;height:100%;">
				<div title="主页" data-options="iconCls:'icon-hamburg-home'" style="padding:10px">
...
<%-- 				   	 	<div class= "out" style="float: left;">
				   	 		...
				   	 		<ul class= "num" style="color: white;">
								<li>1</li>
								<li>2</li>
								<li>3</li>
							</ul>
...
<input type="hidden" value="${sessionScope.employee.department.name}" /> 
</body>
</html>