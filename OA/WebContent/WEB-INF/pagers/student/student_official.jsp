...
<script>
	...
	'"student.stuStatus":"_stuStatus", "student.des":"_des", "student.status":"_status","student.role":"_role","student.room.name":"_roomName","student.grade.name":"_gradeName","student.empId":"_empid"}';
		...
		$.get("<%=path %>/aut/authority?methodName=" + methodName,
			function(data) {
				if (data.resu.request == "success") {
					if (method == "edit") {
						$("#editForm").form("load", JSON.parse(jsonStr)); // 直接获取表单的数据
						$("#editWin").window("open");
		...
		if(rows.length  > 1 ){
			...
			if(row){
				...
				if(row.parentName == 'null'){
					json1 = json1.replace("_parentName",row.parentName);
				}else{
					json1 = json1.replace("_parentName","");
				}
				if(row.parentPhone == 'null'){
					json1 = json1.replace("_parentPhone",row.parentPhone);
				}else{
					json1 = json1.replace("_parentPhone","");
				}
				if(row.parentPhone == 'null'){
					json1 = json1.replace("_parentPhone",row.parentPhone);
				}else{
				...
				if(row.weChat == 'null'){
					json1 = json1.replace("_weChat", row.weChat);
				}else{
					json1 = json1.replace("_weChat", "");
				}
				if(row.address == 'null'){
					json1 = json1.replace("_address", row.address);
				}else{
					json1 = json1.replace("_address", "");
				}
				authority('com.oa.action.StudentAction.stuOfficialUpdate', json1, "edit",'');
	...
	function activate(index){  // 启用
		authority('com.oa.action.StudentAction.stuPurposeActivate', '', "activate",index);
	...
	<table id = "list"class="easyui-datagrid" 
		...
		<thead>
			<tr>
				...
				<th data-options="field:'startDay',align:'center',width:120" formatter="formatterDate">入学时间</th>
	...
	<div id="editWin" class="easyui-window" title="修改正式学生" data-options="closed:true" data-options="iconCls:'icon-save'" style="width:400px;height:440px;padding:5px;">
		<form id = "editForm">
			...
			<tr><td><input type="hidden" name = "student.empId"/></td></tr>
				...
				<tr>
					<td>地址</td>
					<td><input class="easyui-validatebox textbox" name="student.address"  /></td>
				</tr>
				<tr>
					<td>微信</td>
				...
				<tr>
					<td>家长联系方式</td>
					<td><input class="easyui-validatebox textbox" name="student.parentPhone" data-options="validType:'length[11,11]'" /></td>
				</tr>
				<tr>
					<td>家长姓名</td>
					<td><input class="easyui-validatebox textbox" name="student.parentName" data-options="validType:'length[2,10]'"/></td>
				</tr>
				<tr>
					<td>入学时间</td>
