...
<script>
	...
	function showAddGradeWin(){
		authority('com.oa.action.GradeAction.gradeSave', "", "add",'');
	}
	function showEditGradeWin(){
		...
		if(rows.length  > 1 ){
			...
			if(row){
				...
				authority('com.oa.action.GradeAction.gradeUpdate', json1, "edit",'');
			}
	...
	function showAddStuWin(){// 给某个班添加学生
		...
		if(rows.length  > 1 ){
			...
			if(row){
				...
				authority('com.oa.action.StudentAction.addStuOnGrade', '', "addStus",'');
	...
	function disable(index){   // 禁用
		authority('com.oa.action.RoomAction.roomDisable', '', "disable",index);
	}
	...
	function activate(index){  // 启用
		authority('com.oa.action.RoomAction.roomActivate', '', "disable",index);
	}
