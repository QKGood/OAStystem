	...
	function showEditStudentWin(){
		...
		if(rows.length  > 1 ){
			...
			if(row){
				...
				replace("_stuStatus",row.stuStatus).replace("_des",row.des).replace("_status",row.status).replace("_role",row.role).replace("_empid",row.empId).replace("_empid",row.empId);
