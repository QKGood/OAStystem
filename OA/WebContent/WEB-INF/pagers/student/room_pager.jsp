...
<script>
	...
	function showAddRoomWin(){
		authority('com.oa.action.RoomAction.roomSave', "", "add",'');
	...
	function showEditRoomWin(){
		...
		if(rows.length  > 1 ){
			...
			if(row){
				...
				authority('com.oa.action.RoomAction.roomUpdate', json1, "edit",'');
			}
	...
	function activate(index){  // 启用
		authority('com.oa.action.RoomAction.roomActivate', '', "activate",index);
	...
	function showAddStudentWin(){ // 某个宿舍下添加学生
		...
		if(rows.length  > 1 ){
			...
			if(row){
				...
				authority('com.oa.action.StudentAction.addStuOnRoom', '', "addStus",'');
