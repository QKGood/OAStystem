...
<script>
	...
	var jsonStr = '{"notice.noticeId":"_noticeId","notice.noticetype.name":"_noticetype","notice.noticetype.noticetypeId":"_typeId","notice.name":"_name","notice.typeId":"_typeId","notice.des":"_des","notice.noticeDay":"_noticeDay","emp.name":"_name","notice.status":"_status","notice.empId":"_empId"}';
	...
	function showAddNoticeWin() {
		authority('com.oa.action.NoticeAction.addNotice', "", "add",'');
		//类型下拉菜单的级联查询
	...
	function showEditNoticeWin() {
		...
		if (rows.length > 1) {
			$.messager.alert("提示", "请先选择一个需要修改的公告", "info");
			...
			if (row) { // 有选中的行
				...
				replace("_name",row.emp.name).
				replace("_empId",row.empId).
				replace("_noticeDay", row.noticeDay);
				authority('com.oa.action.NoticeAction.updateNotice', jsonStr1, "edit",'');
			}
	...
	function disable(index){   // 禁用
		authority('com.oa.action.NoticeAction.noticeActivate', '', "disable",index);		
	}
		
	function activate(index){  // 启用
		authority('com.oa.action.NoticeAction.noticeDisable', '', "activate",index);	
	}
...
<table id="list" class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'noticeDay',align:'center',width:180" formatter="formatterDate">公告时间</th>
	...
	<div id="addWin" class="easyui-window" title="添加公告" data-options="closed:true" style="width:330px;height:300px;">
		<form id="addForm">
			<table>
				<tr>
					<td>公告标题</td>
...
<!-- 				<tr>
				...
				<tr>
					<td>
						<a href="javascript:;" onclick="addNotice();" class="easyui-linkbutton" data-options="iconCls:'icon-add'" style="width: 65px;height: 30px;margin-left: 1px;" >确认</a>
					</td>
				</tr>
			</table>
		</form>
	...
	<div id="editWin" class="easyui-window" title="修改公告类型" data-options="closed:true" style="width:330px;height:300px;">
		<form id="editForm">
			...
			<table>
				...
				<tr>
					<td>公告描述</td>
					<td><input class="easyui-textbox" data-options="multiline:true"  style="height:100px;" name="notice.des"/></td>
				</tr>
				<tr>
					<td>公告类型</td>
				...
				<!-- 隐藏域  员工ID -->
				<tr><td><input type="hidden" id = "empIds" name = "notice.empId"/></td></tr>
				<tr>
					<td>发布人:</td>
					<td><input  id="empNames" class="easyui-validatebox textbox" name="emp.name" data-options="required:true"   disabled="disabled"/>
					</td>
				</tr>
				<tr>
					<td>公告状态:</td>
				...
				<tr>
					<td>
						<a href="javascript:;" onclick="editNotice();" class="easyui-linkbutton" data-options="iconCls:'icon-add'" style="width: 60px;height: 30px;margin-left: 1px;" >确认</a>
					</td>
				</tr>
			</table>
		</form>
	...
	<div id="selectWins" class="easyui-window" title="选择员工" data-options="closed:true" data-options="iconCls:'icon-save'" style="width:600px;height:350px;padding:5px;">
		...
		<div style="margin-bottom:5px">
				...
				<a href="javascript:;" onclick = "closeEmployees()" class="easyui-linkbutton" iconCls="icon-edit">取消</a>
		</div>
		</div>
	</div>
