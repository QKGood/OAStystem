...
function outExcel(){
	...
	var checkingIds = "";
	  ...
	  var url = "<%=path%>/excel/excelOut?fileName=checkingExcel.xls&checkingIds=" + checkingIds;
...
<body>
	        ...
	        <tr>  
	            ...
	            <s:iterator value="checks" status="s">
	            	<s:if test="#s.getCount()%2==1">
		            	<th field="time<s:property value='#s.getCount()*2-1'/>" width="60" formatter="timeFormate<s:property value='#s.getCount()/2+1'/>">
		            		<s:if test="#s.getCount()==1">
		            			上午
		            		</s:if>
		            		<s:if test="#s.getCount()==3">
		            			下午
		            		</s:if>
		            		<s:if test="#s.getCount()==5">
		            			晚自习
		            		</s:if>
		            	</th>
	            	</s:if>
	...
	<div id= "toolbar" >
		<div align="left" style= "position:absolute;left:10px">
			...
			<a class= "easyui-linkbutton" id="intoExcel" onclick = "showUpload()">导入数据</a>
		</div>
		<div align="right">
