...
function curEdit(value,row,index) {
	...
	if(sessionEmp.role.name!='总经理'){
		return  '<p style="color:#ccc">没有操作权限</p>';
	}else{
		var edit= '';
		if(row.status == 0){
			var del = "<a href='havascript:;' onclick ='updStatus(" + index + ")' style='color:green'>激活</a>";
		}else {
			edit = "<a href='javascript:;' onclick= 'showEdit(" +index + ")' style='color:blue'>修改 </a>";
...
<body id = "body">
    ...
    <thead>  
        <tr>  
            <th data-options="field:'depName',align:'center',width:80" >部门名称</th> 
            <th data-options = "field:'empId',align:'center',width:80,hidden:true" >经理id</th>
            <th data-options="field:'empName',align:'center',width:80">部门经理</th>
            ...
            <th data-options="field:'editde',align:'center',width:80" formatter="curEdit">操作</tr>
        </tr>  
    </thead>  
	</table>  
	...
	<div id="upd_win" class= "easyui-window" title="修改部门信息" data-options="iconCls:'icon-add',closed:true" style="width:300px;height:400px;padding:5px">
		<form id="upd_form" >
			...
			<input name="department.name" type="hidden" />
			<table>
				<tr><td>姓名:</td><td><input disabled="disabled" style="border:0px; background-color:#fff" name="department.name"></td></tr>
