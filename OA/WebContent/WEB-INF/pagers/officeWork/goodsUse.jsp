...
jsonStr = '{"department.depId":"_id","department.name":"_name","department.des":"_des","department.status":"_status","department.employee.empId":"_empId","department.employee.name":"_empName"}';
	...
	var sessionEmp = "<%=session.getAttribute("employee")%>";	/* 获取的是已经登录的员工 */
	var empId = '';
...
function select(value,row,index){
	var row = $("#goodsDatagrid").datagrid("getData").rows[index];
	var a = null;
	if(row.quantity<=0){
		a = "<p>数量不足</p>";
	}else{
		a = '<a style="color:green" onclick = "doSelect('+index+')" >选择</a>';
	}
	return a;
}
...
<body id="body">
	...
	<!-- 添改表 -->
		<!-- 加 -->
	<div id="add_win" class= "easyui-window" title="领用用品" data-options="iconCls:'icon-add',closed:true" style="width:300px;height:400px;padding:5px">
		<form id="add_form" >
			...
			<table>
				...
				<tr><td><a href="javascript:;" onclick="addGoodsUse('<%=path %>/goodsUse/save')" class="easyui-linkbutton" data-options="iconCls:'icon-add'" >确认</a></td><td><a class= "easyui-linkbutton" >取消</a></td></tr>
			</table>
		</form>
	</div>
	<div id = "goods_win" class= "easyui-window" title="物品选择" data-options="closed:true" style="width:500px;height:400px;padding:5px">
		<table  id="goodsDatagrid" class="easyui-datagrid" style="height:100%"  data-options="
